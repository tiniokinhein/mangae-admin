...
import { GROCERY_CATEGORY_FETCH, GROCERY_CATEGORY_UPLOAD, G_CATEGORY } from '../../config/api'
import { db } from '../../config/firebase'
import axios from 'axios'
import { v4 as uuidv4 } from 'uuid'


const EditGCategory = (props) => {
    ...
    const getItem = React.useCallback(() => {
        ...
        .on('value', (snap) => {
            ...
            setTitleMon(values ? values.title_mon : '')
            setTitleMM(values ? values.title_mm : '')
            setImage(values ? values.image : '')
        })
    }, [id])
                    ...
                    previewFile === null ? (
                        image ? (
                            <div className="position-relative">
                                ...
                                <img 
                                    src={GROCERY_CATEGORY_FETCH + image}
                                    alt=""
                                    width="150"
                                />
                            </div>
...
export default withRouter(EditGCategory)
